 ...
 */
void TPatchMeshClasses::reset(
	RtToken type,
	RtInt nu, RtInt nustep, RtToken uwrap,
	RtInt nv, RtInt nvstep, RtToken vwrap)
{
	bool isbicubic   = !strcmp(type,  TRi::RI_BICUBIC);  // bicubic or bilinear patch, defaults to bilinear
	bool isuperiodic = !strcmp(uwrap, TRi::RI_PERIODIC); // wraps periodically or nonperiodically in u direction, defaults to nonperiodic
	bool isvperiodic = !strcmp(vwrap, TRi::RI_PERIODIC); // wraps periodically or nonperiodically in v direction, defaults to nonperiodic
...
/*! reset() calculates the values for the member variables.
 */
void TSubdivisionMeshClasses::reset(RtInt nfaces, RtInt nvertices[], RtInt vertices[]) {
	m_nVerts = 0;
	m_nFaceVerts = 0;
	m_nFaces = nfaces;
...
/*! reset() calculates the values for the member variables.
 */
void TCurvesClasses::reset(
	RtToken type, RtInt ncurves, RtInt nverts[], RtToken wrap, RtInt vstep)
{
	bool isbicubic  = !strcmp(type, TRi::RI_BICUBIC);  // defaults to bilinear
	bool isperiodic = !strcmp(wrap, TRi::RI_PERIODIC); // defaults to non periodic
